...
- НЕ смятай суми и НЕ изчислявай нищо. Само чети записаното.
...
const TOOL = {
  ...
  input_schema: {
    type: "object",
    properties: {
      client_name: { type: ["string", "null"], description: "Име на клиента/фирмата, ако е видимо." },
      ...
      due_date: { type: ["string", "null"], description: "Срок за доставка, YYYY-MM-DD или null." },
...
Deno.serve(async (req: Request) => {
  ...
  const userText = `Това е ${kind}. Разчети я дословно и извикай record_document с извлечените данни. Всеки артикул от таблицата е отделен ред в lines. Ако код на артикул е напъхан в описанието — извади го в client_code.`;
