...
function csJuliaGuide() {
  ...
  const { wrap, close } = erpDialog(`
    ...
    ${S(`1 · Какво прави този екран`)}
    <div style="font-size:14.5px;line-height:1.6">Смята колко ни струва всяко изделие и колко печелим от него. Има ДВЕ себестойности:<br>
    • <b>рецептна</b> — материалите по средни складови цени + операциите по общите им ставки (от рецептата);<br>
    • <b>реална</b> — същите материали + <b>измерените времена</b> от цеховете × <b>ставките</b> „труд+машина+режийни".<br>
    <b>Маржин</b> = продажната цена − реалната себестойност. Данните се извличат веднъж на ден (пише кога) — затова се отваря бързо.</div>

    ${S(`2 · Горната лента — бутон по бутон`)}
    ${R(`Клиент / Изделие`, `избираш чии изделия гледаш. Изделие се търси по код или име.`)}
    ${R(`📋 Изделията на клиента`, `списък на всички изделия на избрания клиент: рецептна и реална себестойност, продажна цена, маржин %. <b>Клик върху ред</b> отваря пълната калкулация. Панелите 🔴 <b>критични пропуски</b> и 🟠 <b>предупреждения</b> показват какво липсва (изделие без цена, операция без време…) — това е работният ти списък.`)}
    ${R(`🧾 Калкулация`, `пълната сметка на ЕДНО изделие: всеки детайл, операция и материал, с време, ставка и цена ред по ред + „Проверки" отдолу.`)}
    ${R(`← Назад към списъка`, `връща от калкулацията към списъка с изделията.`)}
    ${R(`✎ Редакция`, `включва редакцията НАПРАВО в калкулацията: нормовреме и машина на операция, цена на операцията само в тази рецепта, средна цена на материал, продажна цена, „Задай реална цена". Записва се с бутона за запис и се отразява ВЕДНАГА в маржина.`)}
    ${R(`👩 Юлия`, `това упътване — винаги е тук.`)}
    ${R(`⏱ Отчет Времена`, `отваря отчета с ИЗМЕРЕНИТЕ времена от цеховете (същите, които храни калкулацията) — като отделно табче горе, до Себестойности. Само за гледане/справка.`)}
    ${R(`🏭 Операции → Цех`, `списъкът на операциите с ОБЩИТЕ им ставки (€/бр.) и кой цех ги прави. <b style='color:#b91c1c'>Внимание:</b> смяна на ставка тук важи за ВСИЧКИ рецепти — затова пита с червен прозорец.`)}
    ${R(`💲 Ценови листи`, `ПРОДАЖНИТЕ цени по клиенти (наш код → име при клиента + цена). Оттук калкулацията взима цената за маржина. Смяна тук = нов маржин.`)}
    ${R(`⚙️ Разходи и ставки`, `машините (клик върху машина = картонът ѝ, редактируем), заплатите по цехове и режийните → от тях се смята ставката €/час на всеки цех. „🔗 Свързване на машини" казва коя машина от Времената коя счетоводна машина е — за точна ставка. „📥 Запиши себестойностите в операциите" презаписва общите ставки с реалните (прави се само след решение).`)}
    ${R(`🔄 Опресни`, `извлича всички данни наново (времена, покупки, цени). Ползвай го, ако днес са отчетени нови времена или са влезли нови покупки и искаш да ги видиш веднага.`)}
    ${R(`⤓ Excel / 🖨 Печат`, `сваля списъка/калкулацията като Excel или я праща за печат. Нищо не променят.`)}

    ${S(`3 · Иконките ВЪТРЕ в екраните`)}
    ${R(`✋`, `ръчно зададена цена: на цяло изделие („Задай реална цена" — тя ЗАМЕСТВА сметката от рецептата навсякъде) или на един ред (операция с различна цена само в тази рецепта). Видиш ли ✋ — числото е човешко решение, не сметка.`)}
    ${R(`✎ Задай реална цена`, `в Рецептата на изделието (Продукти) и в ✎ Редакция тук: заковаваш себестойността на изделие, когато рецептата е непълна/невярна. Маха се пак оттам — и сметката става автоматична.`)}
    ${R(`✎€`, `върху МАТЕРИАЛ: сменя цената му във ВСИЧКИ рецепти и в склада — пита с червено. Върху ОПЕРАЦИЯ в рецептата: цена САМО за този ред (безопасно).`)}
    ${R(`⚠ жълто`, `нещо липсва: ред без продажна цена, операция без измерено време (брои се 0 и маржинът е завишен), стара покупна цена. Не е грешка — знак къде да попълниш.`)}
    ${R(`🔴 / 🟠`, `критични пропуски / предупреждения в списъка: критичното пречи на сметката (няма цена, няма рецепта), предупреждението я прави неточна (малко измервания, разминаване).`)}
    ${R(`▾ / ▴`, `разгъва/свива подробностите на ред (напр. във Финанси → Маржин).`)}

    ${S(`4 · Как се проверява изделие — стъпка по стъпка`)}
    <ol style="font-size:14.5px;line-height:1.7;margin:4px 0 4px 18px">
      <li>Избери <b>клиента</b> → гледаш „📋 Изделията на клиента";</li>
      <li>Подреди се по <b>маржин %</b> — ниските/червените са за проверка;</li>
      <li><b>Клик на изделието</b> → Калкулация → чети „Проверки" отдолу;</li>
      <li>Липсва време на операция → „✎ Редакция" → напиши <b>нормовреме</b> (пита Григор колко минути отнема) и машината;</li>
      <li>Липсва продажна цена → „💲 Ценови листи" на клиента;</li>
      <li>Запиши → маржинът се преизчислява веднага. Готово.</li>
    </ol>

    ${S(`5 · Златни правила`)}
    <div style="font-size:14.5px;line-height:1.7">
      • Всичко, което пита с <b style="color:#b91c1c">ЧЕРВЕНО</b>, променя МНОГО рецепти наведнъж — чети и при съмнение откажи и питай Данко.<br>
      • ✋ бие сметката: махни ✋, ако искаш пак да се смята само́.<br>
      • Маржин, който изглежда „прекалено добър", често значи операции без време (⚠) — не е истинска печалба.<br>
...
async function erpRenderCostSheet() {
  ...
  v.innerHTML = `
    <div class="erp-toolbar">
      ...
      <button class="btn btn-small btn-primary" id="cs-julia" title="Упътване за работа със Себестойности — всяка иконка какво прави и къде се отразява">👩 Юлия</button>
