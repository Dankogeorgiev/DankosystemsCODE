...
function qcGate(cb) {
  ...
  ovl.innerHTML = `
    <div class="qc-box">
      <p class="qc-title">ПРОВЕРИ ЛИ КАЧЕСТВОТО?<br>РАЗМЕРИТЕ ОТГОВАРЯТ ЛИ?</p>
      <p class="qc-sub">НАПРАВИ КОНТРОЛНО ИЗМЕРВАНЕ<br>И СВЕРКА С ЧЕРТЕЖА.</p>
      ${timed ? `<div class="qc-timer" id="qc-timer">${QC_SECONDS}</div>
        <div class="qc-bar"><div id="qc-bar"></div></div>
        <p class="qc-note">Измери сега — бутонът се отключва след ${QC_SECONDS} секунди.</p>`
        : `<p class="qc-note">Измери детайла и сверѝ с чертежа, преди да отчетеш.</p>`}
